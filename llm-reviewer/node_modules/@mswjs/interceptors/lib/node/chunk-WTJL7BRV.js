...
function defineRawHeaders(headers, rawHeaders) {
  if (Reflect.has(headers, kRawHeaders)) {
    return;
  }
  ...
  Headers = new Proxy(Headers, {
    construct(target, args, newTarget) {
      ...
      if (headersInit instanceof Headers && Reflect.has(headersInit, kRawHeaders)) {
        ...
        defineRawHeaders(headers2, Reflect.get(headersInit, kRawHeaders));
        return headers2;
      }
...
var MockHttpSocket = class extends MockSocket {
  constructor(options) {
    ...
    this.onRequestStart = (versionMajor, versionMinor, rawHeaders, _, path, __, ___, ____, shouldKeepAlive) => {
      ...
      if (this.request.headers.has(_chunkYGM3BCJUjs.INTERNAL_REQUEST_ID_HEADER_NAME)) {
        this.passthrough();
        return;
      }
      this.onRequest({
    ...
    this.onResponseStart = (versionMajor, versionMinor, rawHeaders, method, url, status, statusText) => {
      const headers = parseRawHeaders(rawHeaders);
      const canHaveBody = !_chunkYGM3BCJUjs.RESPONSE_STATUS_CODES_WITHOUT_BODY.has(status);
      if (canHaveBody) {
      ...
      if (writeArgs !== void 0) {
        if (!headersWritten) {
          ...
          const requestHeadersString = rawRequestHeaders.filter(([name]) => {
            return name.toLowerCase() !== _chunkYGM3BCJUjs.INTERNAL_REQUEST_ID_HEADER_NAME;
  ...
  async respondWith(response) {
    ...
    if (_chunkYGM3BCJUjs.isPropertyAccessible.call(void 0, response, "type") && response.type === "error") {
    ...
    if (response.body) {
      try {
        ...
        this.respondWith(_chunkYGM3BCJUjs.createServerErrorResponse.call(void 0, error));
        return;
      }
    } else {
      serverResponse.end();
...
// src/interceptors/ClientRequest/index.ts
var _ClientRequestInterceptor = class extends _chunkYGM3BCJUjs.Interceptor {
  constructor() {
    ...
    this.onResponse = async ({
      ...
      return _chunkTQD7SQGPjs.emitAsync.call(void 0, this.emitter, "response", {
        requestId,
        request,
        response,
        isMockedResponse
...
//# sourceMappingURL=chunk-WTJL7BRV.js.map